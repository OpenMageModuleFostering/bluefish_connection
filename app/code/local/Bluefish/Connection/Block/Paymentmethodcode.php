 ...
 */
class Bluefish_Connection_Block_Paymentmethodcode extends Mage_Adminhtml_Block_System_Config_Form_Field_Array_Abstract
{
    ...
    public function __construct()
    {
        ...
        $this->addColumn('bluestorecode', array(
            'label' => Mage::helper('adminhtml')->__('Bluestore Code'),
            'size'  => 28
        ));
        $this->_addAfter = false;
        $this->_addButtonLabel = Mage::helper('adminhtml')->__('Add Payment Mapping');
        ...
        if($columnName == 'paymentmethod')
        {
			...
			foreach($allActivePaymentMethods as $paymentCode=>$paymentModel)
			{
				...
				if($paymentCode != "paypal_billing_agreement")
					$rendered .= '<option value="'.$paymentCode.'">'.$paymentTitle.'</option>';
			}
